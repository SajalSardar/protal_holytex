...
@section('title', 'Distribute dyeing')
@section('content')
<div class="main-content-container overflow-hidden">
    <div class="d-flex justify-content-between align-items-center flex-wrap gap-2 mb-4">
        <h2 class="mb-0">Distribute dyeing</h2>
        ...
        <nav style="--bs-breadcrumb-divider: '>';" aria-label="breadcrumb">
            <ol class="breadcrumb align-items-center mb-0 lh-1">
                ...
                <li class="breadcrumb-item active" aria-current="page">
                    <span class="fw-medium">Distribute dyeing</span>
                </li>
            </ol>
        </nav>
    </div>
        ...
        <div class=" col-lg-12">
            <div class="card bg-white border-0 rounded-3 mb-4">
                <div class="card-body p-4">
                    ...
                    <form action="{{ route('dyeing.distribute.store') }}" method="POST" enctype="multipart/form-data"
                        id="netting_form">
                        @csrf
                        <div class="row">
                            <input type="hidden" name="dyeing_quotation_id" value="{{ $dyeingquotation->id }}">
                            <input type="hidden" name="order_id" value="{{ $orderDetail->order_id }}">
                            <input type="hidden" name="style" value="{{ $dyeingquotation->style }}">
                            <input type="hidden" name="description" value="{{ $dyeingquotation->description }}">
                            <input type="hidden" name="dyeing_factory_id"
                                value="{{ $dyeingquotation->dyeingFactory->id }}">
                            <div class="col-lg-4 col-sm-6">
                                <div class="form-group mb-4">
                                    <label class="label text-secondary">PO Number</label>
                                    <input name="po_number" id="po_number"
                                        value="{{ old('po_number', $dyeingquotation->po_number) }}" class="form-control"
                                        readonly>
                                    @error('po_number')
                                ...
                                <div class="card border-0 rounded-3 mb-5">
                                    <div class="card-header bg-primary">
                                        <h3 class="text-white" style="text-transform:uppercase">Style: {{
                                            $dyeingquotation->style }}</h3>
                                    </div>
                                    <div class="card-body">
                                        @php
                                        $totalUse = $dyeingquotation->knit_sum + $dyeingquotation->loss_sum +
                                        $dyeingquotation->stock_sum;
                                        @endphp
                                        ...
                                        <table class="table">
                                            <tbody>
                                                ...
                                                <tr>
                                                    <td>{{ $dyeingquotation->description }}</td>
                                                    <td>{{ $dyeingquotation->quantity }} {{ $dyeingquotation->unit }}
                                                        <input type="hidden"
                                                            value="{{ $dyeingquotation->quantity - $totalUse }}"
                                                            id="total_stock_quantity" name="total_stock_quantity">
                                                    </td>
                                                    <td>
                                                        {{ $dyeingquotation->knit_sum ?? 0 }}kg
                                                    </td>
                                                    <td>
                                                        {{ $dyeingquotation->loss_sum ?? 0 }}kg
                                                    </td>
                                                    <td>
                                                        {{ $dyeingquotation->stock_sum ?? 0 }}kg
                                                    </td>
                                                    <td>{{ $dyeingquotation->quantity -
                                                        $totalUse }}kg
                                                    </td>
                                                    <td>Name: {{ $dyeingquotation->dyeingFactory->name }}<br> Address:{{
                                                        $dyeingquotation->dyeingFactory->address }}
                                                    </td>
                                                </tr>
                                            </tbody>
                                        </table>
                                    ...
                                    @if (($dyeingquotation->quantity - $totalUse) > 0)
                                            ...
                                            <div class="col-lg-3 col-sm-3" id="knit_factory_1">
                                                <div class="form-group mb-4">
                                                    ...
                                                    <select class="form-control select2" name="garments_factory_id">
                                                        ...
                                                        @foreach ($garmentsFactory as $items)
