...
@section('title', 'Netting Factroy')
@section('content')
<div class="main-content-container overflow-hidden">
    <div class="d-flex justify-content-between align-items-center flex-wrap gap-2 mb-4">
        <h2 class="mb-0">Netting Factory</h2>
        ...
        <nav style="--bs-breadcrumb-divider: '>';" aria-label="breadcrumb">
            <ol class="breadcrumb align-items-center mb-0 lh-1">
                ...
                <li class="breadcrumb-item active" aria-current="page">
                    <span class="fw-medium">Netting Factory</span>
                </li>
            </ol>
        </nav>
    </div>
        ...
        <div class=" col-lg-12">
            <div class="card bg-white border-0 rounded-3 mb-4">
                <div class="card-body p-4">
                    ...
                    <form action="{{ route('settings.nettingfactroy.store') }}" method="POST">
                        @csrf
                        <div class="row">
                    ...
                    <div class="default-table-area">
                        <div class="table-responsive">
                            <table class="table align-middle">
                                ...
                                <tbody>
                                    @forelse ($nettingFactory as $item)
                                    <tr>
                                        <td>#{{ $item->id }}</td>
                                        <td>{{ $item->name }}</td>
                                        <td>{{ $item->address }}</td>
                                        ...
                                        <td>
                                            <div class="d-flex align-items-center gap-1 justify-content-end">
                                                <a href="{{ route('settings.nettingfactroy.edit',$item->id) }}"
                                                    class="ps-0 border-0 bg-transparent lh-1 position-relative top-2">
                                                    <i class="material-symbols-outlined fs-16 text-body">edit</i>
                                                </a>
                                                <form action="{{ route('settings.nettingfactroy.destroy',$item->id) }}"
                                                    method="POST">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="button" onclick="nettingDelete(this)"
                                                        class="ps-0 border-0 bg-transparent lh-1 position-relative top-2">
                                                        <i
...
@section('script')
<script>
    function nettingDelete(element) {
        Swal.fire({
            title: "Are you sure?",
