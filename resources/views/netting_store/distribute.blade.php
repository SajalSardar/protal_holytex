...
@section('title', 'Distribute Knitting Stock')
@section('content')
<div class="main-content-container overflow-hidden">
    <div class="d-flex justify-content-between align-items-center flex-wrap gap-2 mb-4">
        <h2 class="mb-0">Distribute Knitting Stock</h2>
        ...
        <nav style="--bs-breadcrumb-divider: '>';" aria-label="breadcrumb">
            <ol class="breadcrumb align-items-center mb-0 lh-1">
                ...
                <li class="breadcrumb-item active" aria-current="page">
                    <span class="fw-medium">Distribute Knitting Stock</span>
                </li>
            </ol>
        </nav>
    </div>
        ...
        <div class=" col-lg-12">
            <div class="card bg-white border-0 rounded-3 mb-4">
                <div class="card-body p-4">
                    ...
                    <form action="{{ route('nettingstorestock.knit.distribute.stock.store') }}" method="POST"
                        enctype="multipart/form-data" id="netting_form">
                        @csrf
                        <div class="row">
                            <input type="hidden" name="stock_id" value="{{ $nettingStock->id }}">
                            <input type="hidden" name="style" value="{{ $nettingStock->style }}">
                            <input type="hidden" name="po_number" value="{{ $nettingStock->po_number }}">
                            <input type="hidden" name="order_id" value="{{ $nettingStock->order_id }}">
                            <input type="hidden" name="description" value="{{ $nettingStock->description }}">
                            <div class="col-lg-4 col-sm-6">
                                <div class="form-group mb-4">
                                    <label class="label text-secondary">PO Number</label>
                                    <input id="po_number" value="{{ old('po_number', $nettingStock->po_number) }}"
                                    ...
                                    <div class="card-body">
                                        ...
                                        <table class="table">
                                            <tbody>
                                                ...
                                                <tr>
                                                    <td>{{ $nettingStock->description }}</td>
                                                    <td>{{ $nettingStock->quantity }} {{ $nettingStock->unit }}
                                                        <input type="hidden"
                                                            value="{{ $nettingStock->quantity -$totalUse }}"
                                                    ...
                                                    <td>{{ $nettingStock->quantity - $totalUse }}kg
                                                    </td>
                                                    <td>Name: {{ $nettingStock->storeAddress->name }}<br> Address:{{
                                                        $nettingStock->storeAddress->address }}
                                                    </td>
                                                </tr>
                                            </tbody>
                                        </table>
                                            ...
                                            <div class="col-lg-3 col-sm-3 d-none" id="yarn_dyed_factory_1">
                                                <div class="form-group">
                                                    ...
                                                    <select class="form-control select2" name="dyeing_factory_id">
                                                        ...
                                                        @foreach ($dyeingFactory as $items)
                                                        <option value="{{ $items->id }}">{{ $items->name }}</option>
                                                        @endforeach
                                                    </select>
                                                </div>
